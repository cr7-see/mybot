...
# --- سيرفر Flask الوهمي لإبقاء البوت شغالاً على Render مجاناً ---
app = Flask('')
...
CUSTOM_EMOJI = discord.PartialEmoji(
    name='emoji_12', id=1531847410056171580
)
# ---------------------------------------------------------
  ...
  async def close_ticket(
    ...
    try:
      await interaction.response.send_message(
          'سيتم حذف التذكرة خلال 5 ثوانٍ...', ephemeral=True
      )
...
async def create_ticket_channel(
    interaction: discord.Interaction, prefix: str, title_name: str
):
  ...
  if existing_channel:
    await interaction.followup.send(
        f'لديك تذكرة مفتوحة بالفعل: {existing_channel.mention}', ephemeral=True
    )
    return
  ...
  embed = discord.Embed(
      ...
      description=(
          f'أهلاً بك {user.mention}، يرجى كتابة تفاصيل الطلب أو المشكلة هنا'
          ' وسيرد عليك المسؤول قريباً.'
      ),
  ...
  await interaction.followup.send(
      f'تم إنشاء تذكرتك بنجاح: {ticket_channel.mention}', ephemeral=True
  )
...
class TicketPanelView(discord.ui.View):
  ...
  @discord.ui.button(
      label='طلب هيئة الرقابه والتفتيش',
      emoji=CUSTOM_EMOJI,
      style=discord.ButtonStyle.blurple,
      custom_id='btn_inspect',
      row=1,
  )
  async def btn_inspect(
      self, interaction: discord.Interaction, button: discord.ui.Button
  ):
    await create_ticket_channel(
        interaction, 'رقابة-وتفتيش', 'طلب هيئة الرقابة والتفتيش'
    )
...
class VerifyPanelView(discord.ui.View):
  ...
  @discord.ui.button(
      label='طلب تفعيل حساب',
      emoji=CUSTOM_EMOJI,
      style=discord.ButtonStyle.blurple,
      custom_id='btn_verify',
  )
  async def btn_verify(
      self, interaction: discord.Interaction, button: discord.ui.Button
  ):
...
async def process_verification(guild, target_member, admin_user, channel):
  ...
  try:
    ...
    await channel.send(
        f'✅ تم إعطاء رتبة **مواطن ، RC** وسحب رتبة **جاري تفعيل ، RC** من'
        f' {target_member.mention} وإرسال الرسالة لخاصّه بنجاح!'
    )
  except discord.Forbidden:
    await channel.send(
        f'✅ تم تحديث رتب العضو بنجاح، لكن تعذر إرسال الرسالة للخاص لأن خاصّ'
        ' العضو مغلق.'
    )
...
@bot.tree.command(
    name='verify',
    description='تفعيل العضو المنسوب وإعطائه الرتبة وسحب رتبة الانتظار',
)
@commands.has_permissions(administrator=True)
async def verify_slash(interaction: discord.Interaction, member: discord.Member):
  await interaction.response.defer(ephemeral=True)
  await process_verification(
      interaction.guild, member, interaction.user, interaction.channel
  )
...
async def on_message(message):
  ...
  if message.content.startswith('تفعيل') and message.author.guild_permissions.administrator:
    if message.mentions:
      ...
      await process_verification(
          message.guild, target_member, message.author, message.channel
      )
    else:
      await message.channel.send(
          '❌ يرجى عمل تاق للعضو، مثال: `تفعيل @العضو`'
      )
...
async def setup_tickets(interaction: discord.Interaction):
  ...
  embed = discord.Embed(
      description=(
          'تذاكر السيرفر اختر التكت الذي يناسبك بضغط على زر المناسب أسفل'
          ' شاشة\n\n@everyone'
      ),
      color=discord.Color.red(),
  )
  ...
  await interaction.followup.send(
      'تم إرسال بنل التذاكر العامة بنجاح!', ephemeral=True
  )


@bot.tree.command(
    name='setup_verify', description='إرسال بنل طلب تفعيل الحساب'
)
@commands.has_permissions(administrator=True)
async def setup_verify(interaction: discord.Interaction):
  ...
  duda = discord.Embed(
      ...
      description=(
          'اضغط على الزر بالأسفل لفتح تذكرة طلب تفعيل حسابك في'
          ' السيرفر\n\n@everyone'
      ),
      color=discord.Color.green(),
  )
  ...
  await interaction.followup.send(
      'تم إرسال بنل التفعيل المنفصل بنجاح!', ephemeral=True
  )
...
# تشغيل سيرفر الويب الوهمي أولاً ثم تشغيل البوت
keep_alive()
